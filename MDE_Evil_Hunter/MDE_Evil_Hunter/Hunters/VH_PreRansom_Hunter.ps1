...
function Hunt-RansomNoteDrops {
    ...
    $searchRoots = @("C:\Users\Public","C:\ProgramData") + (Get-VHUserPaths 'Desktop') +
                   (Get-VHUserPaths 'Documents') + (Get-VHUserPaths 'Downloads')
...
function Hunt-MassFileExtensionChange {
    ...
    $searchRoots = (Get-VHUserPaths 'Desktop') + (Get-VHUserPaths 'Documents') + (Get-VHUserPaths 'Downloads')
    foreach ($root in $searchRoots) {
        if (-not (Test-Path $root -EA SilentlyContinue)) {continue}

...
# MDE_Evil_Hunter shared module -- adds Get-VHUserProfiles/Get-VHUserPaths/
# Invoke-VHUserHive so registry + filesystem hunts still cover real users
# when this script is run as SYSTEM via MDE Live Response.
...
function Hunt-RegistryPersistence {
    ...
    # HKLM run paths -- machine-wide, no user context needed
    $hklmRunPaths = @(
    ...
    # HKCU-equivalent run paths -- per user. Running as SYSTEM (Live Response)
    # means HKCU: resolves to the SYSTEM profile, not any real user, so this
    # walks every real user's hive instead (loaded, or offline via NTUSER.DAT).
    $hkcuRunSubPaths = @(
    ...
    Invoke-VHUserHive -Action {
        ...
        foreach ($sub in $hkcuRunSubPaths) {
            $regPath = "Registry::HKEY_USERS\$HiveRoot\$sub"
            if (-not (Test-Path $regPath -ErrorAction SilentlyContinue)) { continue }
            Get-ItemProperty -LiteralPath $regPath -ErrorAction SilentlyContinue |
                ForEach-Object {
                    $item = $_
                    $item.PSObject.Properties | Where-Object { $_.Name -notmatch '^PS' } |
                        ForEach-Object {
                            $val = [string]$_.Value
                            $sev = if ($val -match $suspValuePatterns) { "HIGH" } else { "MED" }
                            Add-Finding -Severity $sev -TechniqueID "T1547.001" -Technique "Registry Run Key (User)" `
                                -Artifact $_.Name -Detail "Value: $val" -Path "HKCU\$sub\$($_.Name)" -User $Profile.Name | Out-Null
                        }
                }
        }
    }
...
function Hunt-StartupFolders {
    ...
    # Machine-wide startup path first ($env:ProgramData is not per-user, safe as-is)
    $startupPaths = @(
        "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp",
        "C:\Users\Public\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup"
    )
    # Per-user startup folders -- $env:APPDATA alone only covers SYSTEM's own
    # profile when run via Live Response, so pull every real user's instead.
    $startupPaths += Get-VHUserPaths 'AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup'

...
function Hunt-CloudExfilStaging {
    ...
    # Cloud sync clients live under the user's profile, so this walks every
    # real user rather than only SYSTEM's own profile.
    $cloudRelPaths = @{
        "OneDrive"    = "OneDrive"
        "Dropbox"     = "Dropbox"
    ...
    foreach ($prof in (Get-VHUserProfiles)) {
        foreach ($cloud in $cloudRelPaths.Keys) {
            $p = Join-Path $prof.ProfilePath $cloudRelPaths[$cloud]

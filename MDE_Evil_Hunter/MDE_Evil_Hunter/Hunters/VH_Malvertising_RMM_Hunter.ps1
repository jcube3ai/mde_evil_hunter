...
# MDE_Evil_Hunter shared module -- adds Get-VHUserProfiles/Get-VHUserPaths/Invoke-VHUserHive
# so this hunter still covers real users when run as SYSTEM via MDE Live Response.
...
$roots = @("C:\Users\Public","C:\Users\Public\Downloads","C:\ProgramData","C:\Windows\Temp") +
         (Get-VHUserPaths 'Downloads') + (Get-VHUserPaths 'Desktop') + (Get-VHUserPaths 'AppData\Local\Temp') +
         $ScanPaths
...
# Per-user Uninstall key (HKCU-equivalent) -- catches software installed for
# a single user rather than machine-wide. HKCU: alone would only see
# SYSTEM's own key when run via Live Response, so this walks every real user.
Invoke-VHUserHive -Action {
    param($Profile, $HiveRoot)
    $regPath = "Registry::HKEY_USERS\$HiveRoot\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*"
    Get-ItemProperty $regPath -EA SilentlyContinue |
        Where-Object {$_.DisplayName -match $rmmPat} |
        ForEach-Object {
            Add-Finding -Severity "MED" -TechniqueID "T1219" -Technique "RMM Installed Software (User)" `
                -Artifact "App: $($_.DisplayName)" `
                -Detail "Version: $($_.DisplayVersion) | Publisher: $($_.Publisher) | InstallDate: $($_.InstallDate)" `
                -Path $_.InstallLocation -User $Profile.Name | Out-Null
            Print-Finding ($script:Findings[-1])
        }
}

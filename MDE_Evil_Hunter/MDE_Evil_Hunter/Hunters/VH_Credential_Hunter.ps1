...
# MDE_Evil_Hunter shared module -- adds Get-VHUserProfiles/Get-VHUserPaths so
# per-user browser/DPAPI/credential-file checks still cover real users when
# this runs as SYSTEM via MDE Live Response.
...
function Hunt-LSASSAccess {
    ...
    $searchPaths=@("C:\ProgramData","C:\Windows\Temp") + (Get-VHUserPaths 'AppData\Local\Temp') + (Get-VHUserPaths 'Downloads')
...
function Hunt-SAMDump {
    ...
    (@("C:\Windows\Temp","C:\ProgramData") + (Get-VHUserPaths 'AppData\Local\Temp') + (Get-VHUserPaths 'Downloads')) | ForEach-Object {
...
function Hunt-DPAPIAbuse {
    ...
    # T1555.004 - Windows Credential Manager vault
    # All of these live under a real user's profile, so this walks every
    # real user on the box rather than relying on $env:APPDATA/$env:LOCALAPPDATA,
    # which under SYSTEM (Live Response) resolve to the SYSTEM profile only.
    foreach ($prof in (Get-VHUserProfiles)) {
        $base = $prof.ProfilePath

        $dpPath = Join-Path $base 'AppData\Roaming\Microsoft\Protect'
        if (Test-Path $dpPath -EA SilentlyContinue) {
            ...
            if ($keys.Count -gt 0) {
                Add-Finding -Severity "INFO" -TechniqueID "T1555.004" -Technique "DPAPI Master Keys" `
                    ...
                    -Path $dpPath -User $prof.Name | Out-Null
            }
        }

        $credDBs = @{
            "Chrome"  = Join-Path $base 'AppData\Local\Google\Chrome\User Data\Default\Login Data'
            "Edge"    = Join-Path $base 'AppData\Local\Microsoft\Edge\User Data\Default\Login Data'
            "Firefox" = Join-Path $base 'AppData\Roaming\Mozilla\Firefox\Profiles'
        }
        foreach ($browser in $credDBs.Keys) {
            ...
            if (Test-Path $p -EA SilentlyContinue) {
                Add-Finding -Severity "MED" -TechniqueID "T1555.003" -Technique "Browser Credential Store" `
                    ...
                    -Path $p -User $prof.Name | Out-Null
            }
        }

        $vault = Join-Path $base 'AppData\Local\Microsoft\Credentials'
        if (Test-Path $vault -EA SilentlyContinue) {
            Get-ChildItem $vault -File -EA SilentlyContinue | ForEach-Object {
                Add-Finding -Severity "MED" -TechniqueID "T1555.004" -Technique "Credential Manager Vault" `
                    -Artifact $_.Name `
                    -Detail "Credential Manager vault entry | Size: $($_.Length) bytes | Modified: $($_.LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss'))" `
                    -Path $_.FullName -User $prof.Name | Out-Null
            }
        }
    }
}
...
function Hunt-CredentialFiles {
    ...
    $suspPaths = @("C:\ProgramData") + (Get-VHUserPaths 'Desktop') + (Get-VHUserPaths 'Documents') +
                 (Get-VHUserPaths 'Downloads') + (Get-VHUserPaths 'AppData\Local\Temp')
